...
def discriminator_loss(y_true,y_pred):
    ...
    return K.mean(K.square(K.flatten(y_pred)- K.concatenate([K.ones_like(K.flatten(y_pred[:BATCH_SIZE,:,:,:])),
                                                                          K.zeros_like(K.flatten(y_pred[:BATCH_SIZE,:,:,:])) ]) ), axis=-1)
...
def train(BATCH_SIZE, epochs = 1000):
    ...
    # if os.path.isfile('./ref_weight/weight/generator.h5'):
    #     generator.load_weights('./ref_weight/weight/generator.h5')
    ...
    # if os.path.isfile('./ref_weight/weight/discriminator.h5'):
    #     discriminator.load_weights('./ref_weight/weight/discriminator.h5')
    ...
    for epoch in range(epochs):
        ...
        for index in range(steps):
            ...
            print("epoch:%d, batch:%d/%d, d_loss:%f, g_loss:%f, time:%f" % (epoch, index, steps, d_loss,g_loss[1], time.time()-batch_star_time))
            step_count += 1

        # generator.save_weights('./ref_weight/weight/generator.h5', True)
        # discriminator.save_weights('./ref_weight/weight/discriminator.h5', True)
...
if __name__ == '__main__':
    ...
    if(args.train):
        train(BATCH_SIZE=BATCH_SIZE,epochs=1)
    else:
