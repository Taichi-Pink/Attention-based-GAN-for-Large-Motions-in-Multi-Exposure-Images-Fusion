...
ch = 3
tfrecord_path      = './tf_records/256_256_tfrecords'
...
def load_data(bs, patch_size):
  ...
  feature_description = {
      'in_LDRs': tf.io.FixedLenFeature([], tf.string),
  ...
  def read_and_decode(example_string):
      ...
      image = tf.io.decode_raw(feature_dict['in_LDRs'], tf.uint8)
      image = tf.cast(image, dtype='float32')
      image = tf.reshape(image, [patch_size, patch_size, ch * 3])
        ...
        while(count):
            ...
            for i in range(bs//10): 
                ...
                for j in range(10):
                      ...
                      plt.imshow(np.squeeze(lab[index:index+1, :, : , 3:6]))
