...
BATCH_SIZE = 20
num_shots = 3
result_path = 'wgan_local_attention/'
sample_path = 'samples/'
log_path = 'log/'
step_count = 0

##test config
test_path = '/home/ziyi.liu1/dataset_new/ICCP19_test_set/Testing_set/'
test_h = 768  # 1024
test_w = 1280  # 1536
batch_size = 1
##train config
train_path = '/home/ziyi.liu1/dataset_new/tf_records/'
img_rows = 256
img_cols = 256
total_images = 20202
steps = 6981
...
def get_input(scene_dir, image_size):
    c = 3
    in_LDR_paths = sorted(glob(os.path.join(scene_dir, 'ghosted/*.tif')))
    ns = len(in_LDR_paths)
    ...
    ref_HDR = get_image(os.path.join(scene_dir, 'GT_HDR.hdr'), image_size=image_size, is_crop=True)
    return in_LDRs, ref_HDR
...
def train(BATCH_SIZE, epochs=1000, attention=False):
    ...
    in_LDRs, ref_HDR = load_data0(filename_queue, BATCH_SIZE, img_rows, img_rows, IN_CH,
                                             num_shots)
    ...
    if os.path.isfile('generator'+'_wgan_local_attention.h5'):
        generator.load_weights('generator' +'_wgan_local_attention.h5')
    ...
    if os.path.isfile('discriminator' +'_wgan_local_attention.h5'):
        discriminator.load_weights('discriminator' +'_wgan_local_attention.h5')
    ...
    for epoch in range(epochs):
        ...
        for index in range(steps):
            ...
            if step_count % 1000 == 0:
                ...
                generator.save_weights('generator' + '_wgan_local_attention' + '.h5', True)
                discriminator.save_weights('discriminator' + '_wgan_local_attention' + '.h5', True)
...
def test(BATCH_SIZE, nice=False, attention=False):
    ...
    optimizer = Adam(0.0002, 0.5)
    generator.compile(loss='mse', optimizer=optimizer)
    generator.load_weights('generator' + suffix+'_wgan_local_attention' + '.h5')
    ...
    for idx in range(0, num_batch):
        ...
        for i, scene_dir in enumerate(batch_scene_dirs):
            _LDRs, _HDR = get_input(os.path.join(test_path, scene_dir), [test_h, test_w])
...
if __name__ == '__main__':
    ...
    # train(BATCH_SIZE=BATCH_SIZE,epochs=1, attention=False)
